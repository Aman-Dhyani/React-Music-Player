...
export default function Music(props) {
  ...
  const reducer = useCallback((count, action) => {
  ...
  // const audio = musicData.music[count].song -- json issue that why i,m using manually imported music by MusicData.jsx
    ...
    <>
      <div id='MusicPage'>
        ...
        <div id='MusicContainerDiv'>
          ...
          <AudioPlayer /*src={audio}*/ src={newAudios[count]}
            onPause={() => updateAnime('paused')}
            onPlay={() => updateAnime('') || updateAnime2('rotating')} />
          ...
          <button
            className='navigaterBtn' id='prev' onClick={() => updateAnime2('') || dispatch({ type: "prev" })}>
          </button>

          <button
            className='navigaterBtn' id='next' onClick={() => updateAnime2('') || dispatch({ type: "next" })}>
          </button>
